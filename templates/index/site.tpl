...
{iterate from=journals item=journal}
	...
	<!-- KB - make title and text link -->
	<h3><a href="{url journal=$journal->getPath()}" class="action titleLink">{$journal->getLocalizedTitle()|escape}</a></h3>
	{if $journal->getLocalizedDescription()}
		<p><a href="{url journal=$journal->getPath()}" class="action textLink">{$journal->getLocalizedDescription()|nl2br}</a></p>
	{/if}

	<!-- KB - hide superfluous links 08-01-14 romc 
	<p><a href="{url journal=$journal->getPath()}" class="action">{translate key="site.journalView"}</a> | <a href="{url 
journal=$journal->getPath() page="issue" op="current"}" class="action">{translate key="site.journalCurrent"}</a> | <a href="{url journal=$journal->getPath() page="user" op="register"}" class="action">{translate key="site.journalRegister"}</a></p>
        -->
	<br/>
{/iterate}
